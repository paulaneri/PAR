...
class Admin extends CI_Controller
{
	...
	function student_bulk_add($param1 = '')
	{
		...
		if ($param1 == 'import_excel')
		{
			...
			redirect(base_url() . 'index.php?admin/mujer/' . $this->input->post('class_id'), 'refresh');
		}
    ...
    function student($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $data['class_id']       = $this->input->post('class_id');
            $data['tc_id']       = $this->input->post('tc_id');
            $data['cd_id']       = $this->input->post('cd_id');
            $data['ingreso_arg']       = $this->input->post('ingreso_arg');
            $data['DU']       = $this->input->post('DU');
            ...
            redirect(base_url() . 'index.php?admin/mujer/' . $data['class_id'], 'refresh');
        }
        if ($param2 == 'do_update') {
          $data['name']           = $this->input->post('name');
          $data['nickname']       = $this->input->post('nickname');
          ...
          $data['hijos']       = $this->input->post('hijos');
            ...
            redirect(base_url() . 'index.php?admin/mujer/' . $param1, 'refresh');
        }
    ...
    function cd($param1 = '', $param2 = '', $param3 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
            if ($param1 == 'create') {
                ...
                redirect(base_url() . 'index.php?admin/cd/', 'refresh');
            }
        if ($param1 == 'do_update') {
            $data['date']        = $this->input->post('date');
          ...
          $data['comentarios'] = $this->input->post('comentarios');
            ...
            $this->db->where('cd_id', $param2);
            $this->db->update('cd', $data);
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?admin/cd/', 'refresh');
        } else if ($param1 == 'personal_profile') {
            $page_data['cd']   = true;
            $page_data['current_cd_id'] = $param2;
        } else if ($param1 == 'edit') {
            $page_data['edit_data'] = $this->db->get_where('cd', array(
                'cd_id' => $param2
            ))->result_array();
        }
        if ($param1 == 'delete') {
        ...
        $page_data['cds']   = $this->db->get('cd')->result_array();
        $page_data['page_name']  = 'cd';
        $page_data['page_title'] = get_phrase('manage_cd');
        $this->load->view('backend/index', $page_data);
    }
        /****MANAGE TC*****/

        function tc($param1 = '', $param2 = '', $param3 = '')
        {
            if ($this->session->userdata('admin_login') != 1)
                redirect(base_url(), 'refresh');
            if ($param1 == 'create') {
                ...
                $data['observamos'] = $this->input->post('observamos');
                $this->db->insert('tc', $data);
                $tc_id = $this->db->insert_id();

                redirect(base_url() . 'index.php?admin/tc/', 'refresh');
            }
            if ($param1 == 'do_update') {
                $data['date']        = $this->input->post('date');
            $data['teacher_id']   = $this->input->post('teacher_id');
              ...
              $data['comentarios'] = $this->input->post('comentarios');
                ...
                $page_data['tc']   = true;
                $page_data['current_tc_id'] = $param2;
            } else if ($param1 == 'edit') {
                $page_data['edit_data'] = $this->db->get_where('tc', array(
                    'tc_id' => $param2
                ))->result_array();
            }
            if ($param1 == 'delete') {
            ...
            $page_data['tcs']   = $this->db->get('tc')->result_array();
            $page_data['page_name']  = 'tc';
            $page_data['page_title'] = get_phrase('manage_tc');
            $this->load->view('backend/index', $page_data);
        }

        function tic($param1 = '', $param2 = '', $param3 = '')
        {
            if ($this->session->userdata('admin_login') != 1)
                redirect(base_url(), 'refresh');
            if ($param1 == 'create') {
                ...
                $data['student_id']   = $this->input->post('student_id');
                $data['comentarios'] = $this->input->post('comentarios');
                $this->db->insert('tic', $data);
                $tic_id = $this->db->insert_id();
            ...
            if ($param1 == 'do_update') {
              ...
              $data['comentarios'] = $this->input->post('comentarios');
                ...
                $this->db->where('tic_id', $param2);
                $this->db->update('tic', $data);
                $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
                redirect(base_url() . 'index.php?admin/tic/', 'refresh');
            } else if ($param1 == 'personal_profile') {
                $page_data['tic']   = true;
                $page_data['current_tic_id'] = $param2;
            } else if ($param1 == 'edit') {
                $page_data['edit_data'] = $this->db->get_where('tic', array(
                    'tic_id' => $param2
                ))->result_array();
            }
            if ($param1 == 'delete') {
            ...
            $page_data['tics']   = $this->db->get('tic')->result_array();
            $page_data['page_name']  = 'tic';
            $page_data['page_title'] = get_phrase('manage_tic');
            $this->load->view('backend/index', $page_data);
        }
        ...
        public function is_member($tc_id, $teacher_id = '')
        {
            if (!is_numeric($teacher_id)) {
                $teacher_id = get_staff_user_id();
            }
        ...
        public function remove_team_member($project_id, $staff_id)
        {
          ...
          function mujer($param1 = '', $param2 = '', $param3 = '')
          {
            if ($this->session->userdata('admin_login') != 1)
                redirect(base_url(), 'refresh');
            if ($param1 == 'create') {
              ...
              $data['roll']           = $this->input->post('roll');
                ...
                redirect(base_url() . 'index.php?admin/mujer/', 'refresh');
            }
            if ($param1 == 'do_update') {
              $data['name']           = $this->input->post('name');
              $data['birthday']       = $this->input->post('birthday');
            ...
            $page_data['page_title'] 	= get_phrase('all_mujeres');
            $page_data['page_name']  = 'mujer';
            $this->load->view('backend/index', $page_data);
          }
