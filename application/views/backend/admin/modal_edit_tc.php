<?php
$edit_data		=	$this->db->get_where('tc' , array('tc_id' => $param2) )->result_array();
foreach ( $edit_data as $row):
?>
<div class="row">
	<div class="col-md-12">
		<div class="panel panel-primary" data-collapsed="0">
        	<div class="panel-heading">
            	<div class="panel-title" >
            		<i class="entypo-plus-circled"></i>
					<?php echo get_phrase('edit_tc');?>
            	</div>
            </div>
			<div class="panel-body">

                <?php echo form_open(base_url() . 'admin/tc/edit/do_update/'.$row['tc_id'] , array('class' => 'form-horizontal form-groups-bordered validate','target'=>'_top'));?>
            <div class="padded">

                <div class="form-group">
                    <label class="col-sm-3 control-label"><?php echo get_phrase('date');?></label>
                    <div class="col-sm-5">
                        <input type="text" class="datepicker form-control" name="date" value="<?php echo $row['date'];?>"/>
                    </div>
                </div>
								<div class="form-group">
										<label class="col-sm-3 control-label"><?php echo get_phrase('tc');?></label>
										<div class="col-sm-5">
												<select name="tc_id" class="form-control">
														<option value=""></option>
														<?php
														$tcs = $this->db->get('tc')->result_array();
														foreach($tcs as $row2):
														?>
																<option value="<?php echo $row2['tc_id'];?>"
																		<?php if($row['tc_id'] == $row2['tc_id'])echo 'selected';?>>
																				<?php echo $row2['name'];?>
																						</option>
														<?php
														endforeach;
														?>
												</select>
										</div>
								</div>
								<div class="form-group">
										<label class="col-sm-3 control-label"><?php echo get_phrase('lugar');?></label>
										<div class="col-sm-5">
												<input type="text" class="form-control" name="lugar" value="<?php echo $row['lugar'];?>"/>
										</div>
								</div>
                <div class="form-group">
                    <label class="col-sm-3 control-label"><?php echo get_phrase('comment');?></label>
                    <div class="col-sm-5">
                        <input type="text" class="form-control" name="comment" value="<?php echo $row['comment'];?>"/>
                    </div>
                </div>
                <div class="form-group">
                    <div class="col-sm-offset-3 col-sm-5">
                      <button type="submit" class="btn btn-info"><?php echo get_phrase('edit_tc');?></button>
                    </div>
                </div>
            </form>
            </div>
        </div>
    </div>
</div>

<?php
endforeach;
?>
